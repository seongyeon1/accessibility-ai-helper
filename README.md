...
- URL을 서버에서 직접 가져와 홈페이지는 HTML 분석과 화면 캡처를 수행하고, PDF·이미지 같은 원격 파일은 문서 분석 흐름으로 자동 분기합니다.
- HTML, 텍스트, 일부 PDF 텍스트 파일을 업로드해 즉시 분석합니다.
- 이미지, HWP, DOCX, 스캔 PDF는 AI 심화 분석 흐름으로 보낼 수 있습니다.
- 압축되었거나 스캔된 PDF는 깨진 바이너리 조각을 보여주지 않고 AI/OCR 기반 심화 분석을 권장합니다.
...
처음 사용하는 흐름과 화면 예시는 [사용가이드](./docs/user-guide.md)를 참고하세요.
